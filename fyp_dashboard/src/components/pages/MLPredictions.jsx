...
const MLPredictions = () => {
  ...
  // Render highlighted code based on line-level attention scores (matches notebook)
  const renderHighlightedCode = (codeLines, lineScores, prediction) => {
    if (!codeLines || !lineScores || codeLines.length === 0) {
      return <div className="no-attention">No attention data available</div>
    }

    // Ensure lineScores array matches codeLines length
    const scores = lineScores.length === codeLines.length 
      ? lineScores 
      : [...lineScores, ...Array(codeLines.length - lineScores.length).fill(0)]

    // Determine colors based on prediction (matches notebook)
    const isVulnerable = prediction === 'VULNERABLE'
    const baseColor = isVulnerable ? '255, 0, 0' : '0, 255, 0'

    // Render lines with highlighting (matches notebook visualization)
    return (
      <div className="attention-code-lines">
        {codeLines.map((line, lineIdx) => {
          const score = scores[lineIdx] || 0
          
          // Cap opacity for readability (matches notebook)
          const alpha = score * (isVulnerable ? 0.7 : 0.5)
          
          // Text color based on opacity
          const textColor = alpha < 0.5 ? 'black' : 'white'
          const numColor = alpha < 0.5 ? '#888' : '#ddd'
            ...
            >
              <span
                style={{
                  ...
                  textAlign: 'right',
                  marginRight: '15px',
                  borderRight: `1px solid ${numColor}`,
                  paddingRight: '5px',
                  userSelect: 'none'
                }}
            >
          ...
          <div className="ml-predictions-list">
            {mlResults.predictions.map((pred, idx) => (
              <div key={idx} className={`ml-prediction-card ${pred.risk_level?.toLowerCase()}`}>
                ...
                {/* Code Highlighting Visualization - Line-level attention (matches notebook) */}
                {pred.code_lines && pred.line_scores && pred.code_lines.length > 0 && (
                  <div className="code-highlighting-section">
                    <div className="code-highlighting-header">
                      <span className="highlight-label">
                        Code Analysis (Darker = Higher Attention per Line)
                      </span>
                    ...
                    <div id={`highlight-${idx}`} className="highlighted-code">
                      {renderHighlightedCode(pred.code_lines, pred.line_scores, pred.prediction)}
                    </div>
                  </div>
                )}
              </div>
